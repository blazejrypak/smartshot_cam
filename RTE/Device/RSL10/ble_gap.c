...
/* ----------------------------------------------------------------------------
 * Function      : GAPM_GenRandAddrCmd(struct gapm_gen_rand_addr_cmd genRandAddrCmd)
 * ----------------------------------------------------------------------------
 * Description   : Sends the initial message to begin the generation of a random
 *                 address
 * Inputs        : - genRandAddrCmd     - Random Address Generation Configuration
 * Outputs       : None
 * Assumptions   : None
 * ------------------------------------------------------------------------- */

void GAPM_GenRandAddrCmd(uint8_t rnd_Type)
{
    struct gapm_gen_rand_addr_cmd *genRandAddrCmd = KE_MSG_ALLOC(GAPM_GEN_RAND_ADDR_CMD,
                     ...
                     ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    switch(msg_id)
    {
        case GAPM_CMP_EVT:
        {
            ...
            if (p->operation == GAPM_RESET)
            {
                ...
                L2CC_Initialize();
            ...
            else if(((p->operation == GAPM_SET_DEV_CONFIG || p->operation == GAPM_CANCEL) && p->status == GAP_ERR_NO_ERROR) ||
                     ...
                     (p->operation == GAPM_CONNECTION_GENERAL)) && p->status != GAP_ERR_NO_ERROR))
            {
...
bool BondList_Add(BondInfo_Type *bond_info)
{
    ...
    if (bondListSize < APP_BONDLIST_SIZE)
    {
        ...
        if (NVR2_full)
        {
            /* Allocate temporary memory to back up NVR2 and add new entry
             * before erasing the sector */
            nvr2Temp = malloc(sizeof(BondInfo_Type) * (bondListSize + 1));
            if(!nvr2Temp)
            {
            ...
            for (uint8_t i = 0; i < SIZEOF_BONDLIST; i++)
            {
                ...
                if (VALID_BOND_INFO(*state))
                {
                    ...
                    memcpy(&nvr2Temp[copied],
                           (uint8_t *)(FLASH_NVR2_BASE +
                                       i * sizeof(BondInfo_Type)),
                           sizeof(BondInfo_Type));

                    /* Update STATE field with new index */
                    nvr2Temp[copied].STATE = copied + 1;
                    copied++;
                }
            }

            /* Add new bond_info to the end of the temporary buffer */
            memcpy(&nvr2Temp[copied], bond_info, sizeof(BondInfo_Type));
            /* Update STATE field with new index */
            nvr2Temp[copied].STATE = copied + 1;
            bond_info->STATE = nvr2Temp[copied].STATE;
            copied++;

            /* Erase NVR2 */
            if (!BondList_RemoveAll())
            {
                /* Free temporary memory used to back up NVR2 (if any) */
                free(nvr2Temp);
                return (false);
            }

            /* NVR2 sector was erased, will write everything to the base address */
            start_addr = (uint32_t *)FLASH_NVR2_BASE;

            /* data_size is the current size + 1 (the new entry) */
            data_size = copied * sizeof(BondInfo_Type);

            /* The data_to_write is nvr2Temp */
            data_to_write = (unsigned int *)nvr2Temp;
        }
        ...
        return (result == FLASH_ERR_NONE); // @suppress("Symbol is not resolved")
    }
    return (false);
}
